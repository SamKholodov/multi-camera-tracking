...
class DatasetSplit:
    ...
    def split_by_pid(
        ...
        """Split samples by PID so identities do not leak across splits."""
        pids = sorted(self.pid_set())
        rng = random.Random(seed)
        rng.shuffle(pids)
        n_val = max(1, int(round(len(pids) * val_fraction))) if pids else 0
        val_pids = set(pids[:n_val])
        train_samples = [s for s in self.samples if s.pid not in val_pids]
        val_samples = [s for s in self.samples if s.pid in val_pids]
        return DatasetSplit(train_samples), DatasetSplit(val_samples)
...
class CombinedReIDDataset:
    ...
    @property
    def datasets(self) -> List[BaseReIDDataset]:
