...
from core.reid.core.preprocessing import DEFAULT_PREPROCESS
...
class PadRatioResize:
    """Pad to W:H=2:1, then resize to target (H, W)."""

    def __init__(self, size: Tuple[int, int], target_wh_ratio: float = 2.0):
        self.target_h, self.target_w = size
        self.target_wh_ratio = target_wh_ratio

    def __call__(self, img: Image.Image) -> Image.Image:
        w, h = img.size
        current_ratio = w / h
        if current_ratio > self.target_wh_ratio:
            new_w = w
            new_h = int(round(w / self.target_wh_ratio))
        else:
            new_w = int(round(h * self.target_wh_ratio))
            new_h = h
        ...
        pad_top = (new_h - h) // 2
        padded.paste(img, (pad_left, pad_top))
        return padded.resize((self.target_w, self.target_h), Image.BILINEAR)

    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}"
            f"(size=({self.target_h}, {self.target_w}), ratio={self.target_wh_ratio})"
        )
...
def _resize_op(img_size: Tuple[int, int], preprocess: str):
    ...
    if preprocess == "pad_ratio_resize":
        return PadRatioResize(img_size)
...
class ViewAwareHorizontalFlip:
    """Horizontally flip an image and swap left/right vehicle view labels."""

    _VIEW_SWAP = {
        2: 5,  # left -> right
        3: 6,  # left front -> right front
        4: 7,  # left rear -> right rear
        5: 2,
        6: 3,
        7: 4,
    }
    ...
    def __call__(
        ...
        if has_view:
            view_id = self._VIEW_SWAP.get(int(view_id), int(view_id))
        return img, view_id
